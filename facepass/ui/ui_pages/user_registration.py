        ...
        if st.button("🔍 Consultar", key="btn_consultar"):
            if email_consulta:
                ...
                if user_controller:
                    ...
                    user_result = user_controller.get_user_by_email(email_consulta)

                    if user_result and user_result.get('success') and user_result.get('data'):
                        user_data = user_result['data']
                        status_text = "✅ Aprovado" if user_data.get('aprovado') else "⏳ Aguardando Aprovação"
                        ...
                        st.info(f"""
                            **Status do Cadastro:**
                            ...
                            - Status: {status_text}
                            - Data de Cadastro: {user_data.get('created_at', 'N/A')}

                            💡 {'Seu cadastro está aprovado! Você já pode usar o reconhecimento facial.' if user_data.get('aprovado') else 'Seu cadastro está em análise. Aguarde a aprovação do gestor.'}
                        """)
                    else:
                        st.warning(f"⚠️ Nenhum cadastro encontrado para o email: {email_consulta}")
                else:
                    st.error("❌ Serviço indisponível no momento.")
            else:
